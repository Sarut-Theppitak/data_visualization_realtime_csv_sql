...
class GUI(Tk):
    ...
    def __init__(self,select_date,select_end_date,GOT_num,cols,*args, **kwargs):
        ...
        self.df = get_df(self.select_date,self.select_end_date,self.GOT_num,self.cols)
        self.str_date = str(self.df.index.date[0])
        self.end_date = str(self.df.index.date[-1])
        self._time = self.df.index.strftime('%Y/%m/%d %X')
        self._data1 = self.df['昇温室ファン']
        self._data2 = self.df['浸炭室ファン']
        self._data3 = self.df['降温室ファン']
        self._data4 = self.df['昇温室ローラー']
        self._data5 = self.df['浸炭室ローラー1']
        self._data6 = self.df['浸炭室ローラー2']
        self._data7 = self.df['浸炭室ローラー3']
        self._data8 = self.df['降温室ローラー']
        self._data9 = self.df['油槽エレベータチェン']
...
#        self.grid_columnconfigure(0, weight=1)
        ...
        task = threading.Thread(target=update_data, args=(self,))
        #this task will be kills if the msin program finish
        task.daemon = True
        tasks.append(task)
...
#        self.grid_columnconfigure(0, weight=1)
        ...
        task = threading.Thread(target=update_data, args=(self.master,))
...
#    select_end_date = [str(datetime.now().date())]
    ...
    ####################################
    if len(GOT_num) == len(select_date):
        try :
            tasks = []
            mygui = GUI(select_date[0],select_end_date[0],GOT_num[0],cols)
            
            for i in range(len(GOT_num)):
                if i != 0:
                    mygui.create_Top(select_date[i],select_end_date[i],GOT_num[i],cols)
            
            for task in tasks:
                task.start()
            #to delete task that is already executed
            observe_task_handle = threading.Thread(target=observe_task)
            observe_task_handle.daemon = True
            observe_task_handle.start()              
        ...
        print('The number of item in GOT_num and select_date is not equal')
        print('Please press enter to exist')
        input()
    exit()
    ####################################
